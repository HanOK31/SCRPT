...
cleaning <- function(text){
    ...
    a2 <- stri_locate_last(text, regex = a1)
    a3 <- which.min(a2[,1])
    a4 <- a1[a3]
    input <- gsub(paste0("(.*)", a4, ".*$"),"\\1", x)
    return(input)
}

makeCorpus <- function(input){
    ##check text columns - may need adjustign with IF statement
    textcorpus <- corpustools::create_tcorpus(input, doc_id = 1:length(input), text_columns = c("abstract", "keywords"))
    textcorpus <- textcorpus$preprocess(remove_stopwords=T)
    for (i in 1:nrow(textcorpus)){
        cleaning(i)
    }
    return(textcorpus)
}
